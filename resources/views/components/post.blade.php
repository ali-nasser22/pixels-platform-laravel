<li class="flex items-start gap-4 not-first:pt-2.5">
    <a class="shrink-0" href="{{ route('profile.show',$post->profile->handle) }}">
        <img alt="avatar" class="size-10 object-cover" src={{$post->profile->avatar_url}}>
    </a>
        ...
        <div class="border-b border-pixel-light/10 pb-5">
            ...
            <div
                ...
                @if($post->isRepost() && $post->content != null)
                    <ul>
                        <x-post
                            :post="$post->repostOf" :show-engagement="false"/>
                    </ul>
                @endif
            </div>
            <!--Action buttons-->
